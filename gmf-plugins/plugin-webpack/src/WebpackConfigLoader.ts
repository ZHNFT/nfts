...
import { URL } from "url";
import { req, Url } from "@nfts/node-utils-library";
...
export type TWebpackDevServerConfigurationFunction = (
...
export class WebpackConfigLoader {
  public static loadConfigFromFile(
    path: string
  ): Configuration | TWebpackConfigurationFunction | undefined {
    if (fs.existsSync(path)) {
      return req.sync(path) as Configuration | TWebpackConfigurationFunction;
    }
  }
  ...
  public static loadDevServerConfigurationFromFile(
    ...
    | DevServerConfiguration
    | TWebpackDevServerConfigurationFunction
    | undefined {
    if (fs.existsSync(path)) {
      return req.sync(path) as
        | DevServerConfiguration
        | TWebpackDevServerConfigurationFunction;
    }
  }

  public static createBasicWebpackConfiguration(
    gmfConfig: IGmfConfig
  ): Configuration {
  ...
  public static async getHttpServerConfig(): Promise<{
    host: string;
    port: string | number;
  }> {
  ...
  private static resolveEntry(entry?: string): string {
    ...
    return entry
      ? path.resolve(appRoot, entry)
      : path.resolve(appRoot, "src/index");
  }
