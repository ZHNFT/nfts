...
describe("execution 方法测试用例", () => {
  test("waterfall execution", () => {
    ...
    const task2 = (count: number): Promise<number> => {
      return new Promise<number>((resolve) =>
        setTimeout(() => resolve(count + 2), 2000)
      );
    };
    ...
    const task4 = (count: number): Promise<number> => {
      return new Promise<number>((resolve) =>
        setTimeout(() => resolve(count + 4), 2000)
      );
    };
    ...
    return expect(
      waterfall<number>([task1, task2, task3, task4, task5], 1)
  ...
  test("serialize execution", () => {
    ...
    return expect(serialize(tasks, undefined))
      .resolves.not.toThrow()
      .then(() => {
  ...
  test("parallel execution", () => {
    const fn = jest.fn();
    const tasks = [fn, fn, fn, fn];
    return expect(parallel(tasks, undefined))
      .resolves.not.toThrow()
      .then(() => {
        expect(fn).toBeCalledTimes(4);
      });
