...
export class ArgumentsParserResult implements IArgumentsParserResult {
  ...
  set command(_: unknown) {
    throw new ArgumentsParserError(
      `command field in ArgumentsParserResult can not be set directly`
    );
  }
  ...
  public addSubCommand(subCommand: string): void {
    if (this._subCommands.has(subCommand)) {
      this._errors.push(
        new ArgumentsParserError(`SubCommandName: ${subCommand} is already defined`)
      );
    }
  ...
  public setValueByParamName = (paramName: string, value: TGoodValueType) =>
